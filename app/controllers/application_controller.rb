  ...
  private
    ...
    def current_user
      ...
      begin          
        ...
        foursquare_quimby = Foursquare::Base.new(session[:access_token])
        @current_user ||= foursquare_quimby.users.find("self")
      rescue Foursquare::InvalidAuth
        nil
      end
    end
    
    def foursquare_quimby
      unless current_user        
    ...
    def foursquare
      unless current_user        
        @foursquare ||= Foursquare2::Client.new(:client_id => Settings.app_id, :client_secret => Settings.app_secret)        
      else        
        @foursquare ||= Foursquare2::Client.new(:oauth_token => session[:access_token])        
      end
    end
